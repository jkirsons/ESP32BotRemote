...
VirtualJoystick.prototype._onTouchStart = function(event) {
  ...
  // forward the action
  var x = touch.pageX - this._container.offsetLeft;
  var y = touch.pageY - this._container.offsetTop;
  return this._onDown(x, y)
}
...
VirtualJoystick.prototype._buildJoystickBase = function() {
  ...
  ctx.lineWidth = 6;
  ctx.arc(canvas.width / 2, canvas.width / 2, canvas.width / 3.5, 0, Math.PI * 2, true);
  ctx.stroke();

  ctx.beginPath();
  ctx.strokeStyle = this._strokeStyle;
  ctx.lineWidth = 2;
  ctx.arc(canvas.width / 2, canvas.width / 2, canvas.width / 2.5, 0, Math.PI * 2, true);
  ctx.stroke();
